...
def get_entities_interactions(entity_df: pd.DataFrame, url_column: str, entity_type: str, interaction_directory_path: str, interaction_type: str, threshold_limit: int, source_column: str, target_column: str, retry_errors: bool=False, write_only_new: bool=False):
    ...
    metadata_file_path = os.path.join(data_directory_path, "metadata_files", f"{entity_type_singular}_url_cols.csv")
    ...
    for _, row in entity_df.iterrows():
        ...
        if (row[count_column] == None) or (row[count_column] == 0):
            ...
            console.print(f"File path: {file_path}", data_directory_path, interaction_directory_path, entity_name, interaction_type, url_column)
            grouped_columns = [original_source_column, target_column]
            console.print(grouped_columns)
            ...
            dfs = []
            additional_data = {'source': row[source_column], 'url_column': row[url_column], 'interaction_type': interaction_directory_path.replace('/', '')}
            ...
            if response is None:
                ...
                if dfs:
                    ...
                    print("final_processed_length", len(final_processed_df))
                    console.print(f"Saving {row[source_column]} for {interaction_type} to {file_path}")
                    final_processed_df.to_csv("test.csv", index=False)
...
if __name__ == "__main__":
    ...
    subset_core_users = subset_core_users[subset_core_users.login == "ZoeLeBlanc"]
    get_entities_interactions(subset_core_users, url_column, entity_type, interaction_directory_path, interaction_type, threshold_limit, source_column, target_column, retry_errors, write_only_new)