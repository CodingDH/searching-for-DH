...
def get_combined_users(users_temp_path):
    """Function to get combined users dataframe. Run this after all users have been added to the temp directory
    :param users_temp_path: path to users temp folder
    :return: combined users dataframe"""
    users_rows = []
    for subdir, _, files in os.walk(users_temp_path):
        for f in files:
            try:
                temp_df = pd.read_csv(subdir + '/' + f)
                users_rows.append(temp_df)
            except pd.errors.EmptyDataError:
                print(f'Empty dataframe for {f}')
    combined_users_df = pd.concat(users_rows)
    combined_users_df = combined_users_df.drop_duplicates(subset=['login', 'id'])
...
def get_new_users(potential_new_users_df, temp_users_dir, users_progress_bar):
    ...
    new_users_df = get_combined_users(temp_users_dir)
