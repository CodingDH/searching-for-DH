...
def get_actors(repo_df, repo_actors_output_path, users_output_path, get_url_field, is_stargazers, is_forks, is_issues):
    ...
    for _, row in repo_df.iterrows():
        try:
            ...
            url = url.replace('?', '?state=all&') if is_issues else url
            if is_stargazers:
            ...
            if len(data_df) == 0:
                ...
                repo_actors_df = data_df.copy() if is_issues else data_df[cols].copy()
                ...
                if 'login' in data_df.columns:
                    if is_issues:
                        ...
                        subset_cols = [col for col in subset_cols if col.startswith('user')]
                        data_df = data_df[subset_cols]
...
def get_repos_user_actors(repo_df,repo_actors_output_path, users_output_path, rates_df, get_url_field, load_existing, is_stargazers, is_forks, is_issues):
        ...
        while len(repo_df[repo_df[get_url_field].notna()]) > calls_remaining:
            ...
            if os.path.exists(repo_actors_output_path):
                ...
                if len(unprocessed_actors) > 0:
                    new_actors_df = get_actors(unprocessed_actors, repo_actors_output_path, users_output_path, get_url_field, is_stargazers, is_forks, is_issues)
                else:
                ...
                repo_actors_df = get_actors(repo_df, repo_actors_output_path, users_output_path, get_url_field, is_stargazers, is_forks, is_issues)
