...
def process_search_data(rates_df, query, output_path, total_results, row_data):
    ...
    searched_df['natural_language'] = row_data['language']
    searched_df['search_type'] = 'tagged' if 'topic' in query else 'searched'
    check_if_older_file_exists(output_path)
    searched_df.to_csv(output_path, index=False)


def process_large_search_data(rates_df, search_url, dh_term, params, initial_output_path, total_results, row_data):
    ...
    for year in years:
        ...
        if year == current_year:
            query = search_url + \
                f"%22{dh_term}%22+created%3A{year}-01-01..{year}-{current_month}-{current_day}+sort:created{params}"
        else:
            query = search_url + \
                f"%22{dh_term}%22+created%3A{year}-01-01..{year}-12-31+sort:created{params}"
        print(query)
        total_pages = int(check_total_pages(query))
        print(f"Total pages: {total_pages}")
        calls_remaining = rates_df['resources.search.remaining'].values[0]
        ...
        search_df['search_term'] = row_data['search_term']
        search_df['search_term_source'] = row_data['search_term_source']
        search_df['natural_language'] = row_data['language']
        search_df['search_type'] = 'tagged' if 'topic' in query else 'searched'
        check_if_older_file_exists(yearly_output_path)
...
def generate_initial_search_datasets(rates_df, initial_repo_output_path,  repo_output_path, repo_join_output_path, initial_user_output_path,  user_output_path, user_join_output_path, org_output_path, overwrite_existing_temp_files=True):
    ...
    cleaned_dh_terms = pd.read_csv(
        '../data/derived_files/cleaned_translated_dh_terms.csv')
    ...
    for _, row in cleaned_dh_terms.iterrows():
        print(f"Getting repos with this term {row.term} in this language {row.natural_language}")
...
if __name__ == '__main__':
    ...
    initial_repo_output_path = "../expanded_search/data/repo_data/"
